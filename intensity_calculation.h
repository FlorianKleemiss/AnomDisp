...
namespace kleemiss_peyerimhoff{
  ...
  double calc_Intensity_s_orbital(double alpha_loc, 
    ...
    for (int l=0; l<=l_max; l++){
      ...
      for (int p=0; p<=p_max; p+=2){
        cvec r = f_a_for_p(el_nr, l, 0, z_temp, nu_in, n0, p);
    ...
    for (int l=0; l<=l_max; l++){
      ...
      for (int p=0; p<=p_max; p+=2){
        for (int nummy=0; nummy < 3; nummy++){
          if (mults[nummy] == 0)
            continue;
          cvec r = f_p_el_for_p(el_nr, l, ms[nummy], ms[nummy], z_temp, nu_in, n0, p);
          for (int run=0; run < r.size(); run++)
    ...
    for (int l=0; l<=l_max; l++){
      ...
      for (int p=0; p<=p_max; p+=2){
        for (int nummy=0; nummy < 5; nummy++){
          if (mults[nummy] == 0)
            continue;
          cvec r = f_d_el_for_p(el_nr, l, ms[nummy], ms[nummy], z_temp, nu_in, n0, p);
          for (int run=0; run < r.size(); run++)
            temp[run] += real(r[run]) * mults[nummy];          
        }
        for (int nummy=0; nummy < 5; nummy++){
  ...
  std::vector<double> calc_stuff(double nu_in, double t0, int l_max, int p_max, int Z){
    if (l_max > 10) err_not_impl_f("L_max not working for bigger than 10",std::cout);
  ...
  double calc_sum(double nu_in, double t0, int Z,int l_max=5, int p_max=1){
    vec parts = calc_stuff(nu_in,t0,l_max,p_max,Z);
    return 2*PI*vec_sum(parts); // factor_x * sum(....)
  }
  
  double at_angstrom(double wl, double t0, int Z,int l_max=5, int p_max=1){
    double nu = speed_of_light / wl *1E10;
    vec parts = calc_stuff(nu,t0,l_max,p_max,Z);
